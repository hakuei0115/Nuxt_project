...
const title = computed(()=> `Freyja | ${roomDetail.value.name} `)
const description = computed(()=> roomDetail.value.description)
const imageUrl = computed(()=> roomDetail.value.imageUrl)
const ogUrl = computed(()=> `https://freyja.travel.com.tw/room/${roomDetail.value.id}`)
</script>
...
<template>
    <Head>
        ...
        <Meta property="twitter:image" :content="roomDetail.imageUrl" />
    </Head>
    <h2>房型詳細頁面</h2>
    <div class="container">
        <button type="button" class="btn btn-primary" @click="router.push('/Day13/room')">回上一頁</button>
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="room-page">
                    <div class="room-header">
                        <h1 class="room-name">{{ title }}</h1>
